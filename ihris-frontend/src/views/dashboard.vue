<template>
  <v-app id="top">
    <the-header app :header="header" v-on:loggedin="updateConfig" />
        ...
        fetch("/config/site").then((response) => {
          response.json().then((data) => {
            if (
              ...
              data.security.hasOwnProperty("disabled")
            ) {
